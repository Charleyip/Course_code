...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    num_mutiples = ((start - stop)//10) + 1

    if num_mutiples <= 0:
        return torch.empty(0, dtype=torch.float64)
    
    tensor = torch.arange(start, stop + 1, 10, dtype=torch.float64)
...
def shuffle_cols(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    if x.shape[1] < 3:
        raise ValueError("Input tensor must have at least 3 columns.")
    # Create an integer array to specify the column order for the output tensor.
    column_order = [0, 0, 2, 1]

    # Use NumPy indexing to construct the output tensor.
    y = x[:, column_order]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    reverse_indices = torch.arange(num_rows - 1, -1, -1, dtype=torch.long)
...
def take_one_elem_per_col(x: Tensor) -> Tensor:
    ...
    if x.shape[0] < 4 or x.shape[1] < 3:
        raise ValueError("Input tensor must have at least 4 rows and 3 columns.")

    # Create a new tensor 'y' by indexing the elements as described
...
def sum_positive_entries(x: Tensor) -> Tensor:
    ...
    pos_sum = int(positive_values.sum())
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return pos_sum
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    # Permute the dimensions to get the desired ordering
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Create a clone of the input tensor
    result = x.clone()
    
    # Use advanced indexing to set the minimum values to 0
    result[torch.arange(x.size(0)), min_indices] = 0
...
def batched_matrix_multiply(
    x: Tensor, y: Tensor, use_loop: bool = True) -> Tensor:
    """
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    B, N, M = x.shape
    B, M, P = y.shape
    z = torch.zeros((B, N, P), dtype = torch.float32)
    ...
    #         for p in range(P):
    #             for m in range(M):
    #                 z[i, n, p] += x[i, n, m] * y[i, m, p]

    return z


def batched_matrix_multiply_loop_with_stack(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    """
    Perform batched matrix multiplication between the tensor x of shape
    (B, N, M) and the tensor y of shape (B, M, P).

    This implementation uses a loop to compute the output and stacks the
    results into a single tensor using torch.stack. The output tensor will
    have the same dtype as the input tensor x.
    ...
    Returns:
        z: Tensor of shape (B, N, P) where z[i] of shape (N, P) is the result
            of matrix multiplication between x[i] of shape (N, M) and y[i] of
            shape (M, P). The output z will have the same dtype as x.
    """
    B, N, M = x.shape
    B, M, P = y.shape
    ...
    for i in range(B):
        # Multiply the i-th matrices in the batch and store the result
        result = torch.mm(x[i], y[i])
        result_list.append(result)
    
    # Stack the results along the batch dimension
    z = torch.stack(result_list, dim=0)
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    mean = torch.sum(x, dim = 0) / x.size(0)

    variance = torch.sum((x-mean)**2, dim = 0)/(x.size(0)-1)
