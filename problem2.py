...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    #activition function
    hidden = torch.relu(X.mm(W1))

    scores = hidden.mm(W2).to(torch.float64)
...
def nn_forward_backward(
    ...
    scores -= scores.max(axis =1, keepdim = True)[0]

    exp_scores = torch.exp(scores)

    softmax = exp_scores / exp_scores.sum(axis = 1, keepdim= True)

    loss = -torch.log(softmax[range(num_samples),y]).sum()
    #scale the losses
    loss /= num_samples
    #l2 regularization 
    loss += 0.5 * reg * (torch.sum(W1 * W1) + torch.sum(W2 * W2))
    ...
    d_scores = softmax
    d_scores[range(num_samples), y] -= 1
    d_scores = d_scores.to(X.dtype)
    d_scores /= num_samples  # Scale by the batch size

    grads['W2'] = torch.mm(h1.T, d_scores)
    grads['W2'] += reg * W2 / N  # Regularization gradient, scaled by N

    d_h1 = torch.mm(d_scores, W2.T)
    d_h1[h1 <= 0] = 0  # ReLU gradient

    grads['W1'] = torch.mm(X.T, d_h1)
    grads['W1'] += reg * W1 / N  
